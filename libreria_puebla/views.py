...
from libreria_puebla.forms import CategoriaForm, ProductoForm,ArticuloForm, Cambiar_precio_form,borrar_libro_form
...
def inicio(request):
    ...
    return render(request,'libreria_puebla/libreria_inicio.html' ,dict_context)
...
def agregar_categoria(request):
    ...
    if request.method == 'POST':
        ...
        if categoria_form.is_valid():
            ...
            return render(request, 'libreria_puebla/agregar_categoria.html', dict_context)
        else:
...
def agregar_libro(request):
    ...
    if request.method == "POST":
        ...
        if libro_form.is_valid():
            ...
            return render(request,'libreria_puebla/agregar_libro.html', context_dict)
        else:
...
def muestra_categoria(request,categoria_nombre_slug):
    ...
    try:
        ...
        dict = []
        for libro in libros:
            lib = Articulo.objects.get(id =int(libro['nombre_libro']),sucursal=1)
    ...
    return render(request,'libreria_puebla/busqueda_categoria.html',context_dict)
...
def caja_ventas(request):
    ...
    context_dict['validador'] = 0
    context_dict['usuario'] = UserProfile.objects.get(user = request.user)
    return render(request,'libreria_puebla/caja_registradora.html', context_dict)

@login_required
def muestra_libro(request, libro_slug):
    context_dict = {}
    try:
        libro = Articulo.objects.get(slug = libro_slug,sucursal=1)
    ...
    return render(request,'libreria_puebla/busqueda_libro.html',context_dict)
...
def borrar_libro(request):
    ...
    if request.method == 'POST':
        ...
        context_dict['validador'] = 0
        context_dict['borrar_form'] = borrar_libro_form()
    return render(request,'libreria_puebla/borrar_libro.html', context_dict)
