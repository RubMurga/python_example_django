...
from libreria.models import Categoria, Producto, Articulo
...
class ArticuloForm(forms.ModelForm):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.fields['nombre'].widget.attrs['placeholder'] = 'Introduce un nombre para el libro'
        self.fields['nombre'].label = 'Nombre del libro a agregar'
        self.fields['nombre'].error_messages['required'] = "Debes introducir un nombre para el libro"
...
class ProductoForm(forms.ModelForm):
    ...
    categoria = forms.ModelChoiceField(queryset = Categoria.objects.all(),widget = forms.Select(attrs={'class':'form-control'}), label = 'Categoria del libro')
    nombre_libro = forms.ModelChoiceField(queryset = Articulo.objects.filter(sucursal=1), widget = forms.Select(attrs={'class':'form-control'}), label = 'Nombre del libro' )
    numero_productos = forms.IntegerField()
    class Meta:
        model = Producto
        fields = ('nombre_libro','costo','porcentaje','precio','numero_productos','categoria','codigo_barras')
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.fields['nombre_libro'].error_messages['required'] = "Debes escoger un articulo"
        self.fields['nombre_libro'].error_messages['invalid'] = "Debes cumplir con el formato"
...
class Cambiar_precio_form(forms.Form):
    producto = forms.ModelChoiceField(queryset = Articulo.objects.filter(sucursal=1),widget = forms.Select(attrs={'class':'form-control'}), label = 'Nombre del libro' )
...
class borrar_libro_form(forms.Form):
    producto = forms.ModelChoiceField(queryset = Articulo.objects.filter(sucursal=1),widget = forms.Select(attrs={'class':'form-control'}), label = 'Nombre del libro a borrar' )

    def __init__(self,*args,**kwargs):
        super(borrar_libro_form,self).__init__(*args, **kwargs)
        self.fields['producto'].error_messages['required'] = "Debes escoger un articulo"
        self.fields['producto'].error_messages['invalid'] = "Debes cumplir con el formato"
