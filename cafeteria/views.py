...
from cafeteria.forms import productosForm, ArticuloForm, Cambiar_precio_form, borrar_producto_form
...
def inicio(request):
    ...
    return render(request,'cafeteria/cafeteria_inicio.html' ,dict_context)
@login_required
def agregar_producto(request):
    ...
    if request.method == 'POST':
        ...
        if agregar_form.is_valid():
            ...
            return render(request, 'cafeteria/agregarproducto.html', dict_context)
        else:
            dict_context['articulo_form'] = ArticuloForm()
...
def agregar_articulo(request):
    ...
    if request.method == 'POST':
        ...
        if articulo_form.is_valid():
            ...
            return render(request, 'cafeteria/agregarproducto.html', context_dict)
        else:
...
def busqueda(request):
    if request.method == 'GET':
        ...
        producto = productos.objects.all().filter(codigo_barras=codigo,sucursal=0)
        if producto:
...
def caja_registradora(request):
    ...
    context_dict['validador'] = 0
    context_dict['usuario'] = UserProfile.objects.get(user = request.user)
    return render(request,'cafeteria/caja_registradora.html', context_dict)
@login_required
def muestro_producto(request, producto_slug):
    ...
    try:
        ...
        context_dict['usuario'] = UserProfile.objects.get(user = request.user)
    return render(request,'cafeteria/busqueda_producto.html',context_dict)
...
def borrar_producto(request):
    ...
    if request.method == 'POST':
        ...
        context_dict['validador'] = 0
        context_dict['borrar_form'] = borrar_producto_form()
    return render(request,'cafeteria/borrar_producto.html', context_dict)

def imprimir(request):
    return render(request, 'cafeteria/probando_imprimir.html')
