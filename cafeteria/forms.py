...
class productosForm(forms.ModelForm):
    ...
    nombre_producto = forms.ModelChoiceField(queryset = Articulo.objects.all(), widget = forms.Select(attrs={'class':'form-control'}), label = 'Nombre del producto' )
...
class Cambiar_precio_form(forms.Form):
    producto = forms.ModelChoiceField(queryset = Articulo.objects.filter(sucursal=0),widget = forms.Select(attrs={'class':'form-control'}), label = 'Nombre del producto' )
