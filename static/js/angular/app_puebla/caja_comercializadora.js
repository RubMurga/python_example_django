...
var caja = angular.module('CRegistradora', []);
  ...
  caja.controller('TodoCtrl', function($scope, $http){
    ...
    $scope.agregarProducto = function(){
          var url = "/Puebla/comercializadora/buscando/caja-registradora/?codigo="+$scope.productoInput;
    ...
    $scope.mandarVenta = function(){
        ...
        for (var i = 0; i < $scope.productos.length; i++) {
          if ($scope.productos[i]['cancelado'] == false) {
            var url = "/Puebla/comercializadora/caja-registradora/venta/?codigo=" + $scope.productos[i]['id'] + "&cancelado=0";
            $http.get(url).success(function (data) {
             ...
             var url = "/Puebla/comercializadora/caja-registradora/venta/?codigo=" + $scope.productos[i]['id']+"&cancelado=1";
            $http.get(url).success(function (data) {
