...
function imprSelec(nombre) {
  ...
  caja.controller('TodoCtrl', function($scope, $http){
    ...
    $scope.agregarProducto = function(){
          var url = "/Chilpancingo/cafeteria/buscando/caja-registradora/?codigo="+$scope.productoInput;
        console.log($scope.productoInput);
    ...
    $scope.mandarVenta = function(){
        ...
        for (var i = 0; i < $scope.productos.length; i++) {
          if ($scope.productos[i]['cancelado'] == false) {
            var url = "/Chilpancingo/cafeteria/caja-registradora/venta/?codigo=" + $scope.productos[i]['id']+"&cancelado=0";
            $http.get(url).success(function (data) {
             ...
             var url = "/Chilpancingo/cafeteria/caja-registradora/venta/?codigo=" + $scope.productos[i]['id']+"&cancelado=1";
            $http.get(url).success(function (data) {
